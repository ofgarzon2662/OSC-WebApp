...
describe('Artifacts List and Creation Flow', () => {
  ...
  const randomKeywords = Array.from({ length: 5 }, () => Math.random().toString(36).substring(2, 10)).join(', ');
  ...
  it('should allow a user to see the artifacts list, create a new artifact, and see it in the list', () => {
    ...
    const keywordSearchInput = 'input[placeholder="Artifact\'s keyword list contains"]';
    ...
    cy.get(keywordSearchInput).clear().type('pineappleHorseRandomLightBulbStreet');
